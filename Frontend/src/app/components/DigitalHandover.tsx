...
export default function DigitalHandover() {
  ...
  return <div className="min-h-screen bg-gray-50"><header className="bg-white border-b border-gray-200 px-4 md:px-8 py-4"><div className="flex items-center gap-4"><Link to="/" className="text-gray-600 hover:text-gray-900"><ArrowLeft size={24} /></Link><h1 className="text-2xl font-bold text-gray-900">Digital Handover</h1></div></header><div className="max-w-2xl mx-auto p-6 space-y-4"><input value={bookingId} onChange={(e) => setBookingId(e.target.value)} type="number" className="w-full border px-3 py-2 rounded" placeholder="Booking ID" /><button onClick={initiatePickup} className="w-full bg-blue-600 text-white py-2 rounded">Initiate Pickup OTP</button>{otpCode && <div className="bg-blue-50 border p-4 rounded">Generated OTP: <strong>{otpCode}</strong></div>}{handoverId && <><input value={verificationCode} onChange={(e) => setVerificationCode(e.target.value)} className="w-full border px-3 py-2 rounded" placeholder="Enter OTP to verify" /><button onClick={verifyPickup} className="w-full bg-green-600 text-white py-2 rounded flex items-center justify-center gap-2"><CheckCircle size={16} />Verify Pickup</button></>}<button onClick={initiateReturn} className="w-full border border-gray-300 py-2 rounded">Initiate Return</button></div></div>;
}
